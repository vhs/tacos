...
router.get(
  '/slack/callback',
  passport.authenticate('slack', { failureRedirect: '/login', successRedirect: '/dashboard' }),
  function (req, res) { res.redirect('/dashboard') }
)

router.get(
  '/slack',
  passport.authenticate('slack', { scope: ['identify', 'groups:read'], team: config.authentication.slack.team })
)
