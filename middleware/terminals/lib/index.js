...
const { deviceStore, loggingStore, terminalStore } = require('../../../lib/stores/')
...
const verifyHMAC = function (req, res, next) {
  ...
  if (!terminalStore.checkTerminalSecured(terminalId)) {
    ...
    Logger.info({ action: 'verifyHMAC', user: '', terminal: terminalId, message: 'Terminal ' + terminalId + ' is missing secret' })
    res.status(403).send(JSON.stringify(res.locals.result))
    ...
    res.locals.result.message = 'error: incorrect message format, missing authentication data'
    Logger.info({ action: 'verifyHMAC', user: '', terminal: terminalId, message: 'Terminal ' + terminalId + ' request had incorrect message format, missing authentication data' })
    ...
    Logger.info({ action: 'verifyHMAC', user: '', terminal: terminalId, message: 'Terminal ' + terminalId + ' request had missing auth data (nonce, ts or hash missing' })
    res.status(403).send(JSON.stringify(res.locals.result))
  } else {
    ...
    if (!terminalStore.verifyHMAC(terminalId, packet)) {
      ...
      Logger.info({ action: 'authenticateRFIDCard', user: '', terminal: terminalId, message: 'Terminal ' + terminalId + ' HMAC verification failed' })
      res.status(403).send(JSON.stringify(res.locals.result))
    } else {
      next()
    }
...
const updateTerminalDescription = function (req, res, next) {
  ...
  if (!req.params.id) {
    ...
    res.locals.result = terminalStore.updateTerminalDescription(terminalId, description)
...
const updateTerminalEnabled = function (req, res, next) {
  ...
  if (!req.params.id) {
    ...
    res.locals.result = terminalStore.updateTerminalEnabled(terminalId, req.body.enabled)
