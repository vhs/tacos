...
const debug = require('debug')('atoms:lib:stores:devices')
...
var DeviceStore = function (dataDir) {
	this.lokiDB = new loki(path.resolve(dataDir, 'devicesStore.json'))
	this.lokiDB.loadDatabase(
		{},
		() => {
			debug("Loading database...")
			debug("Loading devices collection...")
			this.devices = this.lokiDB.getCollection('devices')
			if (this.devices == null) {
				debug("Collection not found!")
				debug("Adding collection!")
				this.devices = this.lokiDB.addCollection(
					'devices',
					{
						indices: ['id'],
						autoupdate: true
